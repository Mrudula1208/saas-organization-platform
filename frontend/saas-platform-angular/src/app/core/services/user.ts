...
export class UserService {
  ...
  updateProfile(fullName: string, profileImageUrl: string): Observable<boolean> {
    ...
    return this.http.put<any>(`${this.apiUrl}/profile`, payload, { headers: this.getHeaders() }).pipe(
      ...
      catchError(() => {
        console.warn('User API updateProfile failed.');
        return of(false);
      })
    );
  }
  ...
  changePassword(currentPassword: string, newPassword: string): Observable<boolean> {
    ...
    return this.http.post<any>(`${this.apiUrl}/change-password`, payload, { headers: this.getHeaders() }).pipe(
      map(res => res.success === true),
      catchError(() => {
        console.warn('User API changePassword failed.');
        return of(false);
      })
    );
  }
